...
const InterpretedTrialResult = ({
  ...
  const handleRequestDeepDive = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_ROOT}/api/request-deep-dive`, {
        method: 'POST',
        headers: {
  ...
  return (
    <li className="mb-4 border border-gray-200 rounded-lg shadow-sm bg-white overflow-hidden"> {/* Added overflow-hidden */}
      ...
      <div className="flex justify-between items-center p-4"> {/* Use consistent padding */}
        ...
        {/* Right side: Status Badge, View Details, and Expand/Collapse Button */}
        <div className="flex items-center flex-shrink-0 gap-2">
          ...
          {onViewTrialDetails && (
            <button
              onClick={() => onViewTrialDetails(item)}
              className="px-3 py-1 text-xs font-medium text-blue-600 bg-blue-50 rounded hover:bg-blue-100 border border-blue-200"
              title="View Full Trial Details"
            >
              View Details
            </button>
          )}
...
const ResultsDisplay = ({ results, patientContext, onPlanFollowups, kanbanTasks, onViewTrialDetails }) => {
        ...
        {results.map((item, index) => {
          ...
          return (
            <InterpretedTrialResult
              ...
              onViewTrialDetails={onViewTrialDetails} // <-- Pass trial details handler
            />
          );
        })}
      </ul>
