...
- **WIWFM therapy-fit handoff** (demo-ready):
  - “Run Efficacy Prediction (WIWFM)” routes to `/validate` (frontend) which renders **`HypothesisValidator`**.
...
- **Deterministic, explainable scoring adjustments (RUO)**:
  - **PARP gate**: penalty vue based on germline status + HRD threshold.
...
### Patient
- **Value**: simple, stepwise experience to translate “germline negative” into “what tumor data matters next”.
...
- **Differentiator**:tion suite + provenance artifacts (reports/figures/sha256).
...
- `oncology-coPilot/oncology-frontend/pothesisValidator.jsx`
...
- **Trials CTA**: replace “Coming Soon” with a real minimal trial shortlist (or an explicit demo-mode stub).
