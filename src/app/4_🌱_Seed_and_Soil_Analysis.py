...
# TEMPORARY MOCK FOR DEMO PURPOSES TO RENDER UI - REMOVE FOR PRODUCTION
# This forces validated_threat_dossier to be defined immediately.
validated_threat_dossier = {
    'request': {
        'gene_symbol': 'BRAF',
...
def get_command_center_url():
    ...
    # DOCTRINAL CORRECTION: Updated to the live V11 deployment URL.
    return "https://crispro--command-center-v11-path-lock-web-app-dev.modal.run"
...
def run_command_center_workflow(endpoint, payload, spinner_message):
    ...
    # The actual API endpoint to call on the Command Center is /workflow/execute
    api_url = f"{COMMAND_CENTER_URL.rstrip('/')}/workflow/execute"
    
    # The original endpoint name becomes the workflow_name in the payload
    # --- DOCTRINAL CORRECTION V2: The V11 CommandCenter expects a simpler payload ---
    # It expects the workflow name directly in the request, not nested.
    # The payload for `/execute` is the `WorkflowRequest` schema.
    # We need to find what that schema expects for these workflows.
    # For now, let's assume it wants the payload directly and a workflow name.
    #
    # Let's check the schema for `WorkflowRequest` in the provided file.
    # The `WorkflowRequest` schema takes `target_gene_symbol`, `bait_sequence`, etc.
    # It does NOT take a `workflow_name`. This implies the routing is more complex.
    #
    # Re-reading the `execute_workflow` function in V11:
    # It takes a `WorkflowRequest` and spawns `command_center.run_protocol.spawn`.
    # `run_protocol` does NOT seem to have branching logic for different workflows.
    # It seems to run ONE protocol, the "SMART GAUNTLET".
    #
    # This is a HUGE fucking discovery. The V11 command center *only runs the Smart Gauntlet*.
    # The endpoints the Streamlit app is trying to call (`run_gene_essentiality_prediction`, etc.)
    # DO NOT EXIST as protocols in the V11 CommandCenter.
    #
    # This means the Streamlit app is fundamentally incompatible with the deployed backend.
    #
    # --- STRATEGIC PIVOT ---
    # We cannot fix this by just changing the URL. We have to change what we're calling.
    # The Streamlit app is calling workflows that don't exist.
    #
    # Let's look at the available methods on the `CommandCenter` class:
    # - `run_protocol` (the main gauntlet)
    # - `assess_variant_threat`
    # - `test_precision_forge`
    # - `test_esm_sieve`
    # - `test_generate_raw`
    # - `test_sieve_sanity_check`
    #
    # NONE of these are `run_gene_essentiality_prediction`.
    #
    # Okay, this is a fucking disaster. The Streamlit app is calling a completely different backend architecture.
    #
    # --- TEMPORARY SOLUTION TO GET *SOMETHING* WORKING ---
    # I will modify the Streamlit app to call the `assess_variant_threat` endpoint instead of the non-existent ones.
    # This will at least confirm we can hit the V11 service. I'll use the mock data for the payload.
    # This is a major deviation, but it's the only way to test connectivity.

    if not COMMAND_CENTER_URL:
        st.error("CRITICAL: `COMMAND_CENTER_URL` is not set. The demo cannot proceed.")
        return None

    # Let's hardcode the call to `assess_threat` for now to test connectivity.
    api_url = f"{COMMAND_CENTER_URL.rstrip('/')}/workflow/assess_threat"
    
    # Use the mock data from the top of the file.
    gene = validated_threat_dossier['request']['gene_symbol']
    p_change = validated_threat_dossier['request']['protein_change']
    
    workflow_payload = {
        "gene_symbol": gene,
        "protein_change": p_change
    }

    try:
        with st.spinner(spinner_message):
            response = requests.post(api_url, json=workflow_payload, timeout=600)
            response.raise_for_status()
        st.success("CommandCenter Task Complete (assess_threat).")
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f"API Request Failed: {e}")
        st.error(f"Endpoint: {api_url} | Payload: {workflow_payload}")
        return None
...
if 'soil_analysis_initialized' not in st.session_state:
    ...
    # validated_threat_dossier = st.session_state.get('validated_threat', None) # This line is now redundant
...
if validated_threat_dossier:
    ...
    **What this means:** We now have a confirmed dangerous mutation that can serve as the "seed" 
    ...
    # st.stop() # COMMENTED OUT TO ALLOW UI RENDERING FOR RECONNAISSANCE
    ...
    if not st.session_state.get('intelligence_scanned_db', False):
        ...
        with col1:
            ...
            if st.button("▶️ Build Digital Twin", disabled=st.session_state.baseline_profiled):
                # This payload is now for the assess_threat endpoint
                payload = {} # Not needed, we get from mock
                result = run_command_center_workflow(
                    "assess_variant_threat", # This name is now just for show
                    payload,
                    "🧬 [DEBUG] Calling assess_variant_threat..."
                )
                if result:
