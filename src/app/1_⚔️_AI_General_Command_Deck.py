...
# --- DOCTRINE: DEFINITIVE PATH CORRECTION ---
# There are two 'tools' directories. We must force Python to use the one inside 'src/'.
# This adds the 'src' directory to the path, ensuring imports resolve correctly.
src_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if src_dir not in sys.path:
    sys.path.insert(0, src_dir)
# --- END CORRECTION ---

# from src.tools.command_center_client import get_all_patients, get_latest_battle_plan_for_patient
...
if selected_patient_id:
    ...
    battle_plan = {
        ...
        "structural_variants": {
            "deletions": [{"gene": "TP53", "location": "chr17:7,571,720-7,590,868"}],
            "insertions": []
        },
        "ensemble_scores": {
            "variant_1": {"AlphaMissense": 0.8, "CADD": 25.3, "PrimateAI": 0.7},
            "variant_2": {"AlphaMissense": 0.2, "CADD": 5.1, "PrimateAI": 0.3}
        }
    }
