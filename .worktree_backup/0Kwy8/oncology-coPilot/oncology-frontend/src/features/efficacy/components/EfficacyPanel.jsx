...
export default function EfficacyPanel({ modelId, mutations, disease, onEfficacyData }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const payload = { model_id: modelId, mutations: backendMuts, options };
        // Add disease if provided (for Therapy Fit universalization)
        if (disease) {
    ...
    run();
  }, [modelId, JSON.stringify(mutations), disease, scoringMode, profileMode, enableFusion]); // re-run on profile changes
