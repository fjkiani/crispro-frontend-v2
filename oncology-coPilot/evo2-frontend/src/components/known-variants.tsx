...
export default function KnownVariants({
  ...
  const analyzeVariant = async (variant: ClinvarVariant) => {
    ...
    const proteinChangeMatch = variant.title.match(/p\.(\w+\d+\w+)/);
    const protein_change = proteinChangeMatch ? proteinChangeMatch[1] : null;
    ...
    if (
      ...
      updateClinvarVariant(variant.clinvar_id, {
        ...variant,
        isAnalyzing: false,
        evo2Error: "Could not parse required variant info from title.",
      });
      return;
    }
