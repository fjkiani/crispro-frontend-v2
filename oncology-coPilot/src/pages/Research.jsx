...
const Research = ({ embedded = false, initialTrials = [] }) => {
  ...
  const [searchResults, setSearchResults] = useState(initialTrials);
  ...
  // Effect to populate initial trials when embedded
  useEffect(() => {
    if (embedded && initialTrials && initialTrials.length > 0) {
      console.log(`[Research] Embedded mode: Loading ${initialTrials.length} initial trials`);
      setSearchResults(initialTrials);
      // Set a default search query to indicate these are ranked trials for the patient
      const patientName = patientData?.demographics?.name || patientId || 'Patient';
      setSearchQuery(`${patientName}'s Ranked Clinical Trials`);
    }
  }, [embedded, initialTrials, patientData, patientId]);

  // --- ENHANCED Handler for Initiating Follow-ups (opens modal) ---
  const handlePlanFollowups = (followupData) => {
    ...
    if (!suggestions || suggestions.length === 0) {
      ...
      // Generate default suggestions for Ayesha's trials
      const defaultSuggestions = [
        {
          suggestion: "Schedule consultation with oncologist to discuss trial eligibility",
          criterion: "Clinical assessment needed",
          draft_text: `Patient interested in trial ${trialId}. Please review eligibility criteria and discuss potential participation.`
        },
        {
          suggestion: "Obtain additional testing required for trial participation",
          criterion: "Pre-screening requirements",
          draft_text: "Coordinate any additional lab work or imaging needed for trial screening."
        },
        {
          suggestion: "Contact trial coordinator for enrollment information",
          criterion: "Enrollment coordination",
          draft_text: `Reach out to trial site coordinator to discuss enrollment process for ${trialTitle}.`
        }
      ];
      console.log(`Using default suggestions for Trial ${trialId}`);
      setModalFollowupData({ suggestions: defaultSuggestions, trialId, trialTitle });
  ...
  // --- Handler to fetch trial details from backend ---
  const fetchTrialDetails = async (nctId) => {
    setIsLoading(true);
    setError(null);
    try {
      // Try new endpoint first: /api/trials/{nct_id}/details
      let apiUrl = `${API_ROOT}/api/trials/${nctId}/details`;
      let response = await fetch(apiUrl);
      
      // Fallback to old endpoint if new one doesn't exist
      if (!response.ok && response.status === 404) {
        apiUrl = `${API_ROOT}/api/trial-details/${nctId}${patientId ? `?patientId=${patientId}` : ''}`;
        response = await fetch(apiUrl);
      }
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ detail: `HTTP error! Status: ${response.status}` }));
        throw new Error(errorData.detail || `Failed to fetch details for trial ${nctId}`);
      }
      
      const result = await response.json();
      // Handle both response formats
      const trialData = result.data || result;
      
      if (trialData) {
        setDetailViewContext({ 
          patientData: patientData || { patientId, demographics: {}, diagnosis: {} }, 
          trialItem: trialData
        });
      } else {
        throw new Error(`Invalid response format when fetching details for trial ${nctId}`);
      }
    } catch (err) {
      console.error(`Error fetching details for trial ${nctId}:`, err);
      setError(`Failed to load details for trial ${nctId}: ${err.message}`);
  ...
  const handleViewTaskDetails = async (task) => {
    ...
    if (!task.trial_id) {
      console.error("Cannot view trial details from task: Missing trial ID on task.");
      alert("Could not load trial details from task: Missing trial ID.");
      return;
    }
    await fetchTrialDetails(task.trial_id);
  };

  // --- Handler to show Detail View when trial is clicked in ResultsDisplay ---
  const handleViewTrialDetails = async (trial) => {
    const nctId = trial.nct_id || trial.id;
    if (!nctId) {
      console.error("Cannot view trial details: Missing NCT ID.");
      setError("Cannot view trial details: Missing NCT ID.");
      return;
    }
    await fetchTrialDetails(nctId);
  };
  ...
  return (
    <div className={embedded ? "p-4" : "container mx-auto p-6 min-h-screen"}>
        {!embedded && (
        ...
        {embedded && (
          <div className="mb-4">
            <h2 className="text-xl font-bold text-blue-600">Enhanced Trial Research</h2>
            <p className="text-sm text-gray-600">Detailed analysis and follow-up planning for clinical trials</p>
          </div>
        )}
