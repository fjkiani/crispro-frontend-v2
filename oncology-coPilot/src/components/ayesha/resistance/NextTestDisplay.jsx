...
const NextTestDisplay = ({ tests }) => {
    ...
    return (
        <Box>
            ...
            {tests.map((test, idx) => {
                const testId = test.test_id || test.test || test.action || `test-${idx}`;
                const priority = test.priority || 'ROUTINE';
                return (
                    <TestCard key={idx} priority={priority}>
                        <CardHeader>
                            <Typography sx={{ fontWeight: 700, color: '#fff' }}>
                                {String(testId).replace(/_/g, ' ')}
                            </Typography>
                            <PriorityBadge label={priority} priority={priority} />
                        </CardHeader>
                        <CardBody>
                            <RationaleText>
                                {test.why || test.description || 'Recommended by engine'}
                            </RationaleText>
                            ...
                            <MetaRow>
                                ...
                                {test.enables_signals && test.enables_signals.length > 0 && (
                                    <Chip
                                        size="small"
                                        label={`Enables Signal ${String(test.enables_signals[0]).split('_')[0]}`}
                            ...
                            <Button
                                ...
                                sx={{
                                    mt: 2,
                                    borderColor: '#4a5568',
                                    color: priority === 'IMMEDIATE' ? '#ff6384' : '#4fd1c5',
