...
export const ChemoGuidanceCard = ({ guidance }) => {
  if (!guidance) return null;
  const score = guidance?.efficacy_score;
  ...
  return (
    <Paper sx={{ p: 2, mb: 2, bgcolor: 'grey.50' }}>
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
        <Typography variant="subtitle1">Chemo Guidance</Typography>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
        {guidance?.disease || '—'} — {guidance?.therapy || '—'}
      </Typography>

      <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap', mb: 1 }}>
        <Box>
          <Typography variant="caption" color="text.secondary">Efficacy</Typography>
