...
const PatientRecordViewer = ({ patientData: initialPatientData }) => {
  ...
  const fetchWorkflow = async () => {
    try {
      const res = await fetch('http://localhost:8000/api/workflow/state');
      if (res.ok) {
  ...
  const handleSaveTrial = async (trial) => {
    try {
      const res = await fetch('http://localhost:8000/api/workflow/save', {
        method: 'POST',
  ...
  const handleTaskMove = async (active, over) => {
    ...
    if (newStatus) {
      ...
      try {
        await fetch('http://localhost:8000/api/workflow/move', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
