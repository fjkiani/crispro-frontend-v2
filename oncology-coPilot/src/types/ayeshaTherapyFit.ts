...
/**
 * Resistance-gate adjustment applied to PARP drugs when SAE features
 * trigger resistance detection. Present only on adjusted drugs.
 */
export interface TherapyFitAdjustment {
  modifier: number;
  risk_level: string;
  reason: string;
}
...
export interface DrugCard {
  ...
  // vNext fields (added 2026-02-18)
  /** Equals efficacy_score when no adjustment applied; adjusted value otherwise. */
  final_score: number;
  /** Present only when resistance gate adjusts this drug's score. */
  therapy_fit_adjustment?: TherapyFitAdjustment;
}
...
/**
 * Machine-readable provenance for pathway score normalization.
 */
export interface PathwayScoresMeta {
  normalized: boolean;
  defaulted: boolean;
  defaulted_keys: string[];
  upstream_keys_present: string[];
...
export interface AnalyzeLevelResponse {
  ...
  // vNext fields (added 2026-02-18)
  pathway_scores_meta?: PathwayScoresMeta;
  completeness?: Record<string, unknown>;
  inputs_used?: Record<string, unknown>;
  /** Canonical ISO-8601 timestamp. */
  analysis_date?: string;
  /** @deprecated Legacy alias for analysis_date. Will be removed in v2. */
  analysisdate?: string;
  /** Present when drug count < 3 (soft warning, not a 500). */
